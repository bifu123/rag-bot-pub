...
| /自动批准入群 | 让机器人是否自动通过用户的加群申请 | 自动通过申请会发送欢迎提示                       |
...
### 插件
 - [查看插件文档](plugin.md)
 插件是本机人最高级的应用，理论是只有python能实现的它都能实现，它可以对接其它数据和业务系统，它是存放于/plugins目录里的python文件或文件夹（如果是文件夹，必须包含有__init__.py文件），分两种模式：串行和并行。串行模式是上一函数的结果是下一函数的参数，最后一个函数结果作为LLM上下文的一部分；并行模式是所有函数返回的结果一并交给LLM。
...
机器人：你的姓名是张三丰
 ...
 ### 文档问答
 让大模型就文档的内容进行回答用户问题，当文档内容不多，在大模型允许的大小以内时，可以直接加载文档，以文档内容回答问题。
 使用方法是：
 - 用命名“/文档问答”切换到文档问答状态
- 向机器人聊天窗口发送文档
- 提出问题，机器人就文档向量内容进行回答
* *在与机器人私聊时所发送的文档为私人所有，在群中所发送的文档为所有群成员共有。

 ### RAG功能
RAG（Retrieval-Augmented Generation，检索增强生成）是用文档对来大模型知识的扩展和外挂，但是与大模型交互时，收发内容的大小是有限的。不可能将很大、很多的文档在问答时发送给它，就得事先在本地进行分割切片索引，将这些索引以向量存储起来，在提交给大模型之前先在本检索近似词。
本机器人RAG功能的使用方法是：
- 用命名“/知识库问答”切换到知识库问答状态
- 向机器人聊天窗口发送文档
- 用命令“/量化文档”对文档向量化存储
- 提出问题，机器人就文档向量内容进行回答
* *在与机器人私聊时所创建的向量库为私人所有，在群中创建的向量库为所有群成员共有。

 ### 本机器人的微信版本
 https://www.github.com/bifu123/wylbot-pub 为本机人的微信版本，在QQ版的基础进行大量优化，将“插件问答”整合到了其它状态。响应更加快速。
